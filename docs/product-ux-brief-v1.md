...
RelayContent คือ Mobile Commerce Content Relay สำหรับครีเอเตอร์ TikTok Shop
Affiliate ทำหน้าที่รับคอนเทนต์จากเครื่องมือ AI หรือไฟล์ของผู้ใช้ แล้วนำมาผูกสินค้า
ตรวจสอบ จัดคิว และเผยแพร่วิดีโอพร้อมปักตะกร้า

> **รับคอนเทนต์ เลือกสินค้า ตรวจความพร้อม ตั้งเวลา แล้ว Relay ไปพร้อมตะกร้า**
...
> เมื่อฉันสร้างคอนเทนต์จากเครื่องมือ AI ที่ใช้อยู่ ฉันต้องการส่งงานนั้นเข้า RelayContent
> เพื่อจับคู่สินค้า ปักตะกร้า ตรวจความพร้อม และตั้งเวลาโพสต์ได้ในที่เดียว โดยไม่ต้อง
> ดาวน์โหลดและตั้งค่าซ้ำหลายรอบหรือเฝ้าระบบจนโพสต์เสร็จ
...
```mermaid
flowchart TD
    A[สมัครหรือเข้าสู่ระบบ] --> B[เชื่อม TikTok Shop Creator]
    B --> C{มีสิทธิ์ Affiliate และ Video Write?}
    C -- ไม่มี --> D[อธิบายเงื่อนไขและวิธีแก้]
    C -- มี --> E[ซิงก์ My Showcase]
    E --> F[เลือกสินค้า]
    F --> G{วิธีเริ่มคอนเทนต์}
    G -->|AI จากสินค้า| H[เลือกแนวคิดและสร้าง Creative Brief]
    G -->|อัปโหลดเอง| I[เลือกวิดีโอ]
    H --> J[AI สร้าง Script / Voice / Subtitle / Video]
    J --> K[ตรวจและแก้ไข]
    I --> K
    K --> L[กำหนด Caption และ Product Anchor]
    L --> M[ตั้งค่า TikTok Shop]
    M --> N{เผยแพร่เมื่อไร}
    N -->|ตอนนี้| O[ยืนยันและเผยแพร่]
    N -->|ตั้งเวลา| P[เลือกวันเวลา]
    P --> O
    O --> Q[Background Worker]
    Q --> R{ผลลัพธ์}
    R -->|สำเร็จ| S[Published + เปิดดูคลิป]
    R -->|แก้เองได้| T[Retry อัตโนมัติ]
    R -->|ต้องใช้ผู้ใช้| U[แจ้งเตือนพร้อม Action]
```
...
### 7.2 Relay AI Content to Shoppable Video

`เชื่อม/Share จาก AI → AI Content Inbox → เลือกงาน → เลือกสินค้า → Review → Caption & Anchor → TikTok Settings → Schedule → Confirm`

การสร้างด้วย AI ภายใน RelayContent เป็นทางเลือกรอง ไม่ใช่เส้นทางหลักของผลิตภัณฑ์
